...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, evaluator,
        ...
        if model_name not in ['TransE', 'DistMult', 'ComplEx', 'RotatE', 'AutoSF', 'PairRE', 'TripleRE', 'InterHT', 'TranS']:
        ...
        if model_name == 'InterHT' and (not double_entity_embedding or not triple_relation_embedding):
            raise ValueError('InterHT should use --triple_entity_embedding and --triple_relation_embedding')

        if model_name == 'TranS' and (not double_entity_embedding or not triple_relation_embedding):
            raise ValueError('TranS should use --triple_entity_embedding and --triple_relation_embedding')
    ...
    def encode_by_index(self, entities: torch.LongTensor) -> torch.FloatTensor:
        ...
        if self.sample_rels > 0:
            ...
            if self.model_name in ['InterHT', 'TranS'] :
    ...
    def PairRE(self, head, relation, tail, mode):
        ...
        score = head * re_head - tail * re_tail + head - tail + re_mid
    ...
    def TripleRE(self, head, relation, tail, mode):
        ...
        re_head = re_head * np.sqrt(re_head.shape[-1])
        re_tail = re_tail * np.sqrt(re_tail.shape[-1])
